...
// ── Tag helpers ───────────────────────────────────────────────────────────
//
// `partialTag` constructs a P-tag for over- or under-saturated applications
// (used by `translateApp` for partial applications).
//
// The companion `conTag`/`funTag` helpers, plus the whole-program
// `generateEvalApply` / `generateEvalFunc` / `generateApplyFunc` machinery
// that lived here, were removed in PR for issue #520: the LLVM backend
// inlines its own eval loop in `translateCase`, so the GRIN-level eval/apply
// definitions were never reached at runtime. Documenting the inlined design
// as the project's eval/apply model is now the canonical answer.

/// Create a partial tag with the given name and missing count.
