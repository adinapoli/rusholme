...
- Backend-owned eval later grew a **second flavour**: `__rhc_perform`
  is the same eval loop without the in-place `Ind` update, so that
  performing a first-class `IO` action value is repeatable. Both are
  generated by one function parameterised on the update discipline —
  see `docs/decisions/939-io-action-representation.md`.
